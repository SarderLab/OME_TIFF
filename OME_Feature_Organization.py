...
for file_path in file_paths:
    # x = 'H14-12555_1L_PAS.segmentations.ome.tiff'
    # base_name should equal 'H14-12555_1L_PAS'
    ...
    for Annotation in root.findall("./Annotation"):
        ...
        if annotationID in all_contours:
            for Region in Annotation.findall("./*/Region"):
                verts = [[int(float(Vert.attrib['X'])), int(float(Vert.attrib['Y']))]
    ...
    for j, obj in enumerate(objects):
        ...
        for i, contours_temp in enumerate(contours_glom):
            if cv2.contourArea(contours_temp) > min_size[j + 2]:
                ...
                min_x, max_x = np.min(contours_temp[:, 0]), np.max(
                    contours_temp[:, 0])
                min_y, max_y = np.min(contours_temp[:, 1]), np.max(
                    contours_temp[:, 1])

                ome_crop = ome_im[min_y:max_y + 1, min_x:max_x + 1]

                edge_points = [
                    (contours_temp[np.where(
                        contours_temp[:, 0] == min_x)][0, 0, :], 1, 0),
                    (contours_temp[np.where(
                        contours_temp[:, 0] == max_x)][0, 0, :], -1, 0),
                    (contours_temp[np.where(
                        contours_temp[:, 1] == min_y)][0, 0, :], 0, 1),
                    (contours_temp[np.where(
                        contours_temp[:, 1] == max_y)][0, 0, :], 0, -1)
                ]

                m_ids = [ome_crop[pt[0][1] + pt[2] - min_y, pt[0]
                                  [0] + pt[1] - min_x] for pt in edge_points]
        ...
        csv_lines = [
            [glom_id_ome[k], file_path.split('/')[-1], 'glomeruli' if j < 2 else objects[j],
             ontology_names[j], doi, tool, ontology_names[j], centroids_x[k], centroids_y[k], 0]
        ...
        if j >= 2:
            concatenated_df = pd.concat([pd.DataFrame(
                [concatenated_df.columns], columns=concatenated_df.columns), concatenated_df], ignore_index=True)
            concatenated_df.columns = range(len(concatenated_df.columns))
            concatenated_df = pd.concat([pd.read_excel(
                template_names[j], header=None), concatenated_df], axis=0, ignore_index=True).fillna('N/A')

        concatenated_df.to_excel(os.path.join(
            outdir, f"{objects[j]}-objects.xlsx"), index=False, header=(j < 2))

    df1 = pd.read_excel(os.path.join(
        outdir, f"{objects[0]}-objects.xlsx"), header=None)
    df2 = pd.read_excel(os.path.join(
        outdir, f"{objects[1]}-objects.xlsx"), header=None).iloc[1:, :]

    df_all = pd.concat([df1, df2], axis=0, ignore_index=True)
    df_all = pd.concat([pd.read_excel(template_names[1],
                       header=None), df_all], axis=0, ignore_index=True)

    scler_col = pd.DataFrame(np.concatenate((np.zeros(
        (len(df1) - 1, 1)), np.ones((len(df2), 1))), axis=0), columns=['Is Sclerotic'])
    scler_col['Is Sclerotic'] = scler_col['Is Sclerotic'].map(
        {1: 'TRUE', 0: 'FALSE'})
    scler_col = pd.concat([pd.read_excel(
        '/orange/pinaki.sarder/haitham.abdelazim/HuBMAP/Templates/glomeruli-combined-template.xlsx', header=None), scler_col], axis=0, ignore_index=True)

    df_all = pd.concat([df_all, scler_col], axis=1).fillna('N/A')
    df_all.to_excel(os.path.join(
        outdir, 'glomeruli-objects.xlsx'), index=False)
